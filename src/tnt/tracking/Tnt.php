...
class Tnt
{
    ...
    {
        $factory = new Factory();
        $client = $factory->create(new Client(), 'http://ws.tntbrasil.com.br/MercurioWS/services/Localizacao?wsdl');
        try {
    ...
    private function buildRequest(): array
    {
        return [
            'localizaMercadoria' => [
                'in0' => [
                    'cnpj' => $this->senderTaxId,
                    'nf' => $this->invoiceNumber,
                    'nfSerie' => $this->invoiceSeries,
...
//            if (!empty($response->out->previsaoEntrega)):
//                $deliveryDate = \DateTime::createFromFormat('d/m/Y H:i:s', $response->out->previsaoEntrega);
//                $currentDate = new \DateTime(date("d-m-Y"));
//                $deliveryDays = $deliveryDate->diff($currentDate)->format("%a");
//            endif;

            $this->result->status = 'OK';
            $this->result->cnpjDevedor = $response->out->cnpjDevedor;
            $this->result->conhecimento = $response->out->conhecimento;
            $this->result->notaFiscal = $response->out->notaFiscal;
            $this->result->pedido = $response->out->pedido;
            $this->result->peso = $response->out->peso;
            $this->result->qtdVolumes = $response->out->qtdVolumes;
            $this->result->previsaoEntrega = $response->out->previsaoEntrega;
            $this->result->localizacao = $response->out->localizacao;
            $this->result->dataEntrega = $response->out->dataEntrega;
            $this->result->motivoNaoEntrega = $response->out->motivoNaoEntrega;
